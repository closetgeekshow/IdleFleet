...
document.addEventListener("alpine:init", () => {
  Alpine.data("app", () => ({
    ...
    heartBeat() {
      ...
      for (let ship of this.ships) {
        ...
        if (!ship.available) {
          if (new Date() > ship.returnTime) {
            ...
            this.addLog(
              `${ship.name} returned and earned ${this.numberFormat(
                moneyEarned
              )} credits.`
            );
      ...
      if (hasNextShip) {
        this.nextShipReturnTime =
          Math.max(Math.floor(((new Date() - nextShipResult) / 1000) * -1), 0) +
          " seconds";
      }
    },
    ...
    addShip() {
      ...
      let newShip = {
        ...
        trip() {
          ...
          this.tripDuration = getRandomInt(
            constants.DURATION_MIN,
            constants.DURATION_MAX
          );
          ...
          if (mainThat.shipSpeed >= 2) {
            let percentSavings = Math.min(
              getRandomInt(1, mainThat.shipSpeed),
              95
            );
            //console.log('return time was ', this.tripDuration);
            this.tripDuration -= Math.floor(
              this.tripDuration * (percentSavings / 100)
            );
    ...
    randomEvent() {
      ...
      if (whatHappened < 40) {
        ...
        let msg =
          this.messages.moneyWon[
            getRandomInt(0, this.messages.moneyWon.length)
          ] + ` Gain ${this.numberFormat(moneyWon)} credits!`;
        ...
        let msg =
          this.messages.moneyLost[
            getRandomInt(0, this.messages.moneyLost.length)
          ] + ` Lose ${this.numberFormat(moneyLost)} credits.`;
        this.credits -= moneyLost;
        this.addLog(`<strong class="bad">${msg}</strong>`);
      } else if (whatHappened < 92) {
        let msg =
          this.messages.shipWon[getRandomInt(0, this.messages.shipWon.length)];
        this.addLog(`<strong class="good">${msg}</strong>`);
        this.addShip();
      } else {
        if (this.ships.length < 10) return;
        let msg =
          this.messages.shipLost[
            getRandomInt(0, this.messages.shipLost.length)
          ];
        this.addLog(`<strong class="bad">${msg}</strong>`);
    ...
    get rank() {
      ...
      return constants.RANKS[
        pos > constants.RANKS.length - 1 ? constants.RANKS.length - 1 : pos
      ];
    },
      ...
      try {
        ...
        if (savedGame) {
          ...
          this.ships = gameState.ships.map(ship => 
            this.createShip(ship.name, ship.available, ship.returnTime)
          );
    ...
    createShip(name, available = true, returnTime = null) {
      let mainThat = this;
      return {
        name,
        available,
    ...
    // Modified addShip method
  addShip() {
    const name = this.generateShipName();
    const newShip = this.createShip(name);
    this.ships.push(newShip);
    this.addLog(`${newShip.name} acquired.`);
  }
  }));
